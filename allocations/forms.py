...
class CombinedRegistrationForm(forms.Form):
    ...
    phone_number = forms.CharField(
        label='Linha', max_length=20, required=False)
    iccid = forms.CharField(label='ICCID', max_length=22, required=False)
    carrier = forms.CharField(
        label='Operadora', max_length=100, required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        text_fields = [
            'full_name', 'corporate_email', 'employee_id', 'department',
            'phone_number', 'iccid', 'carrier'
        ]
        for name in text_fields:
            self.fields[name].widget.attrs.setdefault('class', 'form-control')
        self.fields['status'].widget.attrs.setdefault('class', 'form-select')
        self.fields['line_action'].widget.attrs.setdefault(
            'class', 'form-check-input')
        # RadioSelect renders inputs; we set class on widget but Django applies per input automatically for RadioSelect
