...
class DashboardView(AuthenticadView, TemplateView):
    ...
    def _build_status_counts(self):
        ...
        for row in SIMcard.objects.filter(is_deleted=False) \
                .values('status') \
                .annotate(count=Count('id')):
            sim_counts[row['status']] = row['count']

        for row in PhoneLine.objects.filter(is_deleted=False) \
                .values('status') \
                .annotate(count=Count('id')):
        ...
        return {
            'sim_status_counts': [
                {
                    'label': label,
                    'count': sim_counts.get(value, 0)
                } for value, label in SIMcard.Status.choices
            ],
            'line_status_counts': [
                {
                    'label': label,
                    'count': line_counts.get(value, 0)
